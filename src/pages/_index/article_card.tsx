...
export default class ArticleCard extends React.Component<IArticle> {
  ...
  render(): React.ReactNode {
    const { title, description, cover, id, date, view, like, comment } = this.props;
    const articleUrl = `/article/${id}/detail`;
    // debugger
    return <>
      <Card title={title} extra={<Link to={articleUrl}>More</Link>}
            hoverable={true}
        // cover={<img src={cover}></img>}
            actions={[
              <IconText icon={CalendarOutlined} text={date} key='list-vertical-star-o' />,
              <IconText icon={EyeOutlined} text={view} key='list-vertical-star-o' />,
              <IconText icon={LikeOutlined} text={like} key='list-vertical-like-o' />,
              <IconText icon={MessageOutlined} text={comment} key='list-vertical-message' />,
            ]}
      ...
      >
        <Row gutter={[16, 24]}>
          ...
          <Col span={cover?18:24}><p>  {description}</p></Col>
        </Row>
